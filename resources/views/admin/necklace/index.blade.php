	...
	<div class="row">
		<div class="col-sm-12">
	      ...
	      <tbody>
	          ...
	          <td>
                <form class="del" action="/necklace/{{$v->id}}" method="post">
                {{method_field('DELETE')}}
                {{csrf_field()}}
                <a class="btn btn-info btn-sm pull-left" href="/necklace/{{$v->id}}/edit">修改</a>
	...
	<div class="row">
		<div class="col-sm-12">
			<div class="dataTables_paginate paging_simple_numbers pull-right" id="datatable-fixed-header_paginate">
				{{ $necklace->appends(['num'=>$num, 'keywords'=>$keywords])->links() }}
			</div>
		</div>
	</div>
	</div>
