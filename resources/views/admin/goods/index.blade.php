	...
	<div class="row">
		<div class="col-sm-12">
	        ...
	        <tr role="row">
	        	...
	        	<th class="sorting text-center" tabindex="0" aria-controls="datatable-fixed-header" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 259px;">商品价格
	        	</th>
	        	<th class="sorting text-center" tabindex="0" aria-controls="datatable-fixed-header" rowspan="1" colspan="1" aria-label="Office: activate to sort column ascending" style="width: 117px;">库存
	        	</th>
	      ...
	      <tbody>
	          ...
	          <td>{{$v->kucun}}</td>
	          <td>
	          	<a class="btn btn-info btn-sm pull-left" href="/goods/{{$v->id}}/edit">修改</a>
                <form class="del" action="/goods/{{$v->id}}" method="post">
                {{method_field('DELETE')}}
                {{csrf_field()}}
